...
- [x] 6.5.6 通用弹窗组件 AppModal（Teleport、缩放动画、插槽）
- [x] 6.5.7 种子数据完善（在线图片 URL、头像、文件记录统一并入 seed.sql）
- [x] 6.5.8 首页交互修复（时间筛选移除、查看更多导航、封面图片显示）
- [x] 6.5.9 资源详情页重写（点赞/收藏状态高亮、分享复制、评分悬停预览、Markdown渲染、头像统一）
- [x] 6.5.10 搜索筛选修复（分类展开子分类、标签SQL子query、minRating连接、NL搜索关键词优化）
- [x] 6.5.11 阿里云 OSS 文件存储集成（头像/资源文件上传，storage.type 切换）
