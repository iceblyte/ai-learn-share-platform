...
| TC-INT-010 | 点赞 | 历史点赞数保持累加 | 资源存在预置点赞数 | 1. 查看资源点赞数 2. 点击点赞 | 点赞数在原值基础上 +1，不会重置为 1 | P0 |
| TC-INT-011 | 评论 | 评论失败提示 | 未登录或接口报错 | 1. 尝试发表评论 | 前端显示明确错误提示 | P1 |

## 4. AI 功能模块 (P1)

| 用例ID | 模块 | 用例名称 | 前置条件 | 测试步骤 | 预期结果 | 优先级 |
|--------|------|---------|---------|---------|---------|--------|
| TC-AI-001 | 摘要 | 生成资源摘要 | 资源存在 | 1. POST /api/v1/ai/summary | 返回约100字摘要；AI 超时或未配置时不阻塞接口 | P1 |
| TC-AI-002 | NL搜索 | 自然语言搜索 | 已登录 | 1. POST /api/v1/search/nl ("推荐关于Java并发编程且评分最高的前5个资源") | 返回解析意图；AI 不可用时仍通过本地规则返回相关结果 | P1 |
| TC-AI-003 | 推荐 | 获取个性化推荐 | 已登录，有浏览历史 | 1. GET /api/v1/ai/recommendations?page=1&size=10 | 返回分页推荐列表+推荐理由 | P1 |
| TC-AI-004 | 推荐理由 | AI 推荐理由降级 | 已登录，存在资源 | 1. POST /api/v1/ai/recommendations/reasons | AI 不可用时返回本地可解释推荐理由 | P1 |
| TC-AI-005 | 聊天 | 悬浮聊天流式回复 | AI_KEY 有效且模型可用 | 1. 打开悬浮聊天窗 2. 提问 | 返回真实 AI 回复并以分块形式逐步显示 | P1 |
| TC-AI-006 | 聊天 | 鉴权失败提示 | AI_MODEL 无权限或无额度 | 1. 打开悬浮聊天窗 2. 提问 | 返回明确失败提示，不返回伪造离线答复 | P1 |
